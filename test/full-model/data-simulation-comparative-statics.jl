...
@testset "simulate all data" begin
    ...
    @test_broken ys[1] < ys[6] # FAILS - αψ up ⟹ more exploration, more exhaustion

    expired = end_ex0(wp) + 1
    exhausted = end_inf(wp)
    @test xterms[1][expired] < xterms[3][expired] # cheap expiration ⟹ more expire

    @test_broken xterms[1][expired] > xterms[6][expired]     # FAILS: αψ ↑ ⟹ LESS expired
    @test_broken xterms[1][exhausted] < xterms[6][exhausted] # FAILS: αψ ↑ ⟹ MORE exhausted
